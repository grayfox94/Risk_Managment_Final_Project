...
def Kelly(prob_p, payoff_p, prob_q, payoff_q):
    #General binary Kelly betting scheme
    p = prob_p
    q = prob_q
    a = payoff_p
    b = abs(payoff_q) #this payoff is actually how many units you lose, 
    
    Kelly_fraction = (p*a - b*q)/(a*b)

    return Kelly_fraction

def Kellyapprox(mean, variance, skewness, kurtosis):

    #this will solve the taylor expansion of the utility function to maximize log growth
    #of form ax^3 + bx^2 + cx^3 + d
    a = (-1)*kurtosis
    b = skewness
    c = (-1)*variance
    d = mean
...
def moments(p,q,payoffp,payoffq):
    #binary events
    p = p
    q = q
    a = payoffp
    ...
    array = (mean,var,skew,kurt,M5)
    return array

def main(p,q,payoffp,payoffq):
    x = moments(p,q,payoffp,payoffq)
    print(x)
    print(Kelly(p,payoffp,q,payoffq))
    print((Kellyapprox(x[0],x[1],x[2],x[3])))
    print(x[0]/x[1])
    return
